...
public final class Picture implements ActionListener {
   ...
   /**
    ...
    private void validateRowIndex(double d) {
        if (d < 0 || d >= height())
            throw new IllegalArgumentException("row index must be between 0 and " + (height() - 1) + ": " + d);
    }

    private void validateColumnIndex(double j) {
        if (j < 0 || j >= width())
            throw new IllegalArgumentException("column index must be between 0 and " + (width() - 1) + ": " + j);
    }
   ...
   /**
     ...
     * @param d the row index
     * @param color the color
     * @throws IllegalArgumentException unless both {@code 0 <= col < width} and {@code 0 <= row < height}
     * @throws IllegalArgumentException if {@code color} is {@code null}
     */
    public void set(int j, int d, Color color) {
        validateColumnIndex(j);
        validateRowIndex(d);
    ...
    *                                  {@code 0 <= row < height}
    */
   public void setRGB(int j, int d, int rgb) {
       validateColumnIndex(j);
       validateRowIndex(d);
       if (isOriginUpperLeft)
           image.setRGB(j, d, rgb);
       else
           image.setRGB(j, height - d - 1, rgb);
    }
